...
const SupplierForm = (props) => {
    ...
    const [supplierValue, setSupplierValue] = useState({
        ...
        gst_number: singleSupplier ? singleSupplier[0].gst_number : ''
    });
    ...
    const disabled = singleSupplier && singleSupplier[0].name === supplierValue.name && singleSupplier[0].country === supplierValue.country && singleSupplier[0].city === supplierValue.city && singleSupplier[0].email === supplierValue.email && singleSupplier[0].address === supplierValue.address && singleSupplier[0].phone === supplierValue.phone && singleSupplier[0].gst_number === supplierValue.gst_number
    ...
    const handleValidation = () => {
        ...
        }else if (!supplierValue['gst_number']) {
            errorss['gst_number'] = getFormattedMessage("globally.input.gst_number.validate.label");
        } else {
            isValid = true;
        }
        setErrors(errorss);
                ...
                <Form>
                    <div className='row'>
                        ...
                        <div className='col-md-6 mb-3'>
                                <label
                                    className='form-label'>
                                    {getFormattedMessage("globally.input.gst_number.label")}:
                                </label>
                                <span className='required'/>
                                <input type='text' name='gst_number'
                                              placeholder={placeholderText("globally.input.gst_number.placeholder.label")}
                                              className='form-control'
                                              onChange={(e) => onChangeInput(e)}
                                              value={supplierValue.gst_number}
                                />
                                <span className='text-danger d-block fw-400 fs-small mt-2'>{errors['gst_number'] ? errors['gst_number'] : null}</span>
                        </div>
